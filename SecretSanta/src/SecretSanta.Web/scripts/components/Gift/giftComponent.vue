    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr v-for="gift in gifts" v-bind:id="gift.id">
                ...
                <td>{{gift.userId}}</td>
                <td>
                    <button class="button is-light" @click='setGift(gift)'>Edit</button>
                    <button class="button is-primary" @click='deleteGift(gift)'>Delete</button>
                </td>
            </tr>
            
        </tbody>
    </table>
        <gift-details-component v-if="this.selectedGift != null"
...
<script lang="ts">
    ...
    export default class GiftsComponent extends Vue {
        ...
        async deleteGift(gift: Gift) {
            let giftClient = new GiftClient();
            if (confirm(`Are you sure you want to delete ${gift.title} ${gift.description} ${gift.url}?`)) {
